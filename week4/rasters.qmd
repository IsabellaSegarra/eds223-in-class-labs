...
class(zion_elevation) # [rows x columns x layers]
...
```{r}
landsat3 <- subset(landsat, 3) # Subset with layer number
landsat4 <- subset(landsat, "landsat_4") # Subset with name
...
merged <- merge(austria, switzerland) # Need to be the same resolution
...
map1 <- tm_shape(austria) + 
  tm_raster(col.legend = tm_legend("Elevation (m)"), 
            col.scale = tm_scale_continuous(values = c(0,1000,2000,3000)) + # Update breaks 
...
```{r}
global(zion_elevation, sd) # Not using a sample 
...
# Cell ID (moves from reading order)
zion_elevation[1]
...
# Row 1, Column 1
zion_elevation[2,3] # Look at dimensions of the data 
...
```{r}
landsat[1] <- cbind(c(0), c (0), c(0), c(0)) # Need to match the number of layers 
landsat[1]
...
# Check NA values 
global(is.na(test_raster), "sum") # (is.na(), "function")
...
```{r}
map1 <- tm_shape(zion_elevation) +
  tm_raster(col.legend(title = "Elevation (m)")) +
...
```{r}
zion_elevation + zion_elevation # Doubling each cells values (check max value)

zion_elevation^2 # Square the raster data

log(zion_elevation)

zion_elevation >5 # determines whether eahc cell has a value greater than 5
...
rcl <- matrix(c(1000, 1500, 1, # group 1 ranges from 1000 - 1500 m
                ...
                ncol = 3, byrow = TRUE)
rcl # want to apply this matrix to your data

# Use reclass matrix to reclassify elevation raster
reclassified <- classify(zion_elevation, rcl = rcl)

# Change reclassified values into factor 
values(reclassified) <- as.factor(values(reclassified))

summary(reclassified) # Tells us the amount of cells that are in grpup 1 (values that range from 100 to 1500 m)
...
```{r}
# Define NDVI (ratio for assessing vegetation)

ndvi_fun <- function(nir,red) {
  (nir - red) / (nir + red)
}

# Apply NDVI function to Landsat bands 3&4 (order of arguments come from function Landsat 4 before landsat 3)
...
```{r}
# Adding empty cells to the left and right 
...
```{r}
zion_elevation + elev_2 # Error: extents don't match, mismatch of cells 
```
...
```{r}
origin(zion_elevation) #returns coordinates of origin 
...
```{r}
# Disaggregate the aggregated raster (cannot recover original data)

zion_elevation_disagg <- disagg(zion_elevation_coarse, fact = 5, method = "bilinear") # fact = 5, splits pixels from 1x1 to 5x5
