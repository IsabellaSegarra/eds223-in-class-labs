...
# Subset nz_height to just the features that intersect Cantebury 

c_height1 <- nz_height[canterbury, ] # [rows, columns] # shows the 70 elevations in Cantebury, NZ
...
# Check that interesect is opposite of disjoing
...
```{r}
# st_filter() same as dplyr::filter()
...
```{r}
# Sparse binary predicate list (returning empty or 1)

nz_height_sgbp <- st_intersects(x = nz_height, y = canterbury)
nz_height_sgbp # Returning 'empty' if elements are not there, can turn this into a logical operator (T/F)

# To make useful, conver to logical vector 

nz_height_logical <- lengths(nz_height_sgbp) > 0 
nz_height_logical # returns true/false
...
tm_shape(canterbury) +
  tm_fill(fill = "darkgrey") +
tm_shape(c_height4) +
  tm_dots(fill = "red") # within 1000km of Canterbury
```
...
# Specify join based on geometries within x and y 
...
nz_height_left_join <- st_join(nz_height, nz) %>% 
  select(id = t50_fid, elevation, region = Name) # rename, NEW = OLD
...
nz_height_left_join %>% 
  ...
  st_drop_geometry() #speeds up process since we are not using
```
...
```{r}
tm_shape(cycle_hire_join) +
  tm-symbols(fill = "red", fill_alpha = 0.2)
```
...
```{r}
# Same as above, but with aggregate function
...
map1 <- tm_shape(nz_elevation_join) +
  tm_polygons(fill = "elevation", 
              fill.legened = tm_legend(title = "Mean elevation (meters)")) +
...
aggregating_zones_area_weighted <- st_interpolate_aw (x = incongruent_2, 
                                                       aggregating_zones,
                                                       extensive = TRUE) # what we are aggreagting is a pop or count, false is for density/rate of population
...
nz_pos <- st_point_on_surface(nz) # point on surface
