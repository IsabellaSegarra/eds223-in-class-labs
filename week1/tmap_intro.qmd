...
tm_shape(world_vector) + # defines the spatial object to plot
  tm_polygons() # defines how to plot the object (can also use tm_dots() for dotted lines)
...
tm_shape(world_vector) + # defines the spatial object to plot
  tm_polygons() + # defines how to plot the object (can also use tm_dots() for dotted lines)
tm_shape(world_cities) +  # plots the world cities 
  tm_dots()
```
...
# `is.main = TRUE` keeps the extend of that shape, w/o it keeps sthe first layer 
...
# indentation is for aesthetic reasons 
...
# Fill polygon with a color palette representing a variable from the data
library(tmap)
tm_shape(world_vector) +
...
```{r}
tm_shape(world_cities) +
  tm_bubbles() # tm_symbols/squares/(shape = ##)/dots/bubbles
...
# adding specifications for range of tm_scale

fill.scale = tm_scale(breaks = c(0,5000,2000,5000, Inf),
