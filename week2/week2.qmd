...
```{r}
st_crs(london_sf) # Look at output
```
...
In mapping, CRS stands for Coordinate Reference System, which is a framework that defines how real-world locations are represented on a map by providing a standardized way to specify a point's position and relate it to the Earth's 3D surface. It includes components like a datum (a model of the Earth), a coordinate system (like latitude and longitude or projected X/Y grids), and a mathematical projection to flatten the 3D surface onto a 2D map, preventing misinterpretations and ensuring accurate placement of data.  
...
world_df <- world %>% 
  select(-geom) # This does not actually remove the geom column
...
```{r}
lond_sf_transform <- st_transform(london_sf, crs = st_crs(london_proj))
...
```{r}
if (st_crs(lond_sf_transform) == st_crs(lond_proj)){
  print("it's a match!")
} else {
...
if (st_crs(dataset1) != st_crs(dataset2)) {
  warning("coordinate refrence sytems don't match!")
...
```{r}
world_transformed <- transform(world, crs = 8857)

# Converting crs of data 
...
# What would happen if you didn't drop the geometry? 
  # Would return the geom polygon 
```
...
# Helpful when perofrming left/right joins 
```
